...
volatile bool cap_index=0,cap_index_1;
volatile int queue_count=0,queue_count_1=0;
...
void PWM_QEI_Init(void){  
   
		SYSCTL_RCGCPWM_R |= 0x00000001; // activate PWM0 clock
    SYSCTL_RCGCQEI_R|=(1<<0);       //activate QEI clock
    ...
    GPIO_PORTD_LOCK_R = 0x4C4F434B ;   //Unlock PD7
		...
		GPIO_PORTD_AFSEL_R |= (1<<PD0)|(1<<PD1)|(1<<PD6)|(1<<PD7);  // enable PD0,PD1,PD6,PD7 pin alternative functionality
		GPIO_PORTB_AFSEL_R |= (1<<PB4)|(1<<PB5);  // enable PB4 and PB5 pin alternative functionality	
    GPIO_PORTD_PCTL_R = (GPIO_PORTD_PCTL_R&0x00FFFF00)|0x66000044;  // page 1344
		GPIO_PORTB_PCTL_R = (GPIO_PORTB_PCTL_R&0xFF00FFFF)|0x00440000;  // replace portE to portD	
    GPIO_PORTD_AMSEL_R &= ~((1<<PD0)|(1<<PD1)|(1<<PD6)|(1<<PD7)); // disable analog functionality PD0,PD1,PD6,PD7.
		GPIO_PORTB_AMSEL_R &= ~((1<<PB4)|(1<<PB5)); // disable aternative functionality PB4 and PB5	
    GPIO_PORTD_DEN_R |= (1<<PD0)|(1<<PD1)|(1<<PD6)|(1<<PD7); // enable i/o on PD0,PD1,PD6,PD7
...
void QEI_Init(void)
{
 ...
 // GPIO_PORTD_AMSEL_R |= (1<<PD7)|(1<<PD6);        // 3) disable analog function
